...
function playconfirm1(value){
    return function(){
        ...
        var input1 = prompt("식사하신 등심의 수를 입력해주세요")
        var input2 = prompt("식사하신 안심의 수를 입력해주세요")
        var sum = input1 * 18000 + input2 * 19000
        alert("총 금액은 " + sum + "원입니다")
        let coupon = confirm("쿠폰을 사용하시겠습니까?")
        var cnumber = 0;
        if(coupon){
            var cnumber = 1;
            sum = sum - 10000;
        ...
        if(con){
            ...
            .then((res) => {
                if(res.success){
                    alert("결제 되었습니다.")
                    location.href = "/ownerReservation"
                }
                else{
            ...
            .catch((err) => {
                console.error(new Error("결제 중 에러발생"));
            });
        }
    }
};
